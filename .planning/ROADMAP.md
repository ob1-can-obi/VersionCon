...
- [ ] **Phase 7: Cloud Mode + Relay Server** - Relay deployment, JWT auth, CloudTransport, same UX as LAN over internet (13/13 plans complete; commits 6ebafa0..29d812f span Wave 4; awaiting /gsd-code-review 7 + /gsd-verify-work 7 before phase close. Wave 1: 07-01 ✓, 07-02 ✓, 07-03 ✓; Wave 2: 07-04 ✓, 07-05 ✓, 07-06 ✓, 07-07 ✓; Wave 3: 07-08 ✓ relay skeleton, 07-09 ✓ relay auth, 07-10 ✓ relay limits + grace timer, 07-11 ✓ relay logger; Wave 4: 07-05b ✓ host wiring + CloudHostTransport demultiplexer + envelope.target unicast + relay first-frame carve-out + per-joiner JWT, 07-12 ✓ deploy artifacts — Dockerfile multi-stage non-root + fly.toml force_https no-auto-stop + relay/README.md 189-line Deploy Your Relay quickstart + top-level README cloud section; 996 / 0 / 66 extension + 71 relay tests passing; ROADMAP SC-3 closed; SC-1 + SC-2 paper-verifiable + deploy story underneath; NET-06 feature-complete)
...
- [x] 07-01 Transport interface + LanTransport refactor (commits 800c233 RED + c0ba05f GREEN; 884 tests pass byte-identically)
- [x] 07-02 CloudEnvelope type + wrap/unwrap (byte-shape snapshot pinned for future L3 forward-compat; commits 973d071 + 6f8ac80 + ebba324)
...
- [x] 07-10 Relay limits.ts (30/min IP sliding-window + 1000 sessions cap + 50 members/session cap + 1 MiB maxPayload + 30-min idle reaper + 60s host-drop grace timer) — commits 5f7aa26 RED + 1fad3ae GREEN + 2433c61 REFACTOR; 13 new relay tests (12 in limits.test.js + 1 env-var override in limits.env.test.js); 44 relay / 973 extension all green; T-07-06 / T-07-07 / T-07-08 / T-07-13 / T-07-14 all mitigated; pure-policy split — limits.ts has zero imports + zero console calls; new SessionRegistry methods scheduleGracePeriod / cancelGracePeriod / closeSession / allSessions (Rule 2 additions documented in 07-10-SUMMARY.md)
- [x] 07-11 Relay logger.ts (pino + redact config; snapshot test: Bearer / payload / code never appear; structured-logger migration of server.ts + auth.ts) — commits 97c6db4 (logger.ts) + 2e42bb0 (logger.test.js, 19 tests) + bcfc4cf (server.ts + auth.ts migration); 63 relay / 973 extension all green; T-07-03 (Bearer leak) + T-07-04 (payload leak) + T-07-05-aux (host-side-secret leak) + T-07-04-aux (err.stack/message leak) all mitigated; remove:true semantics pinned (key absence, not '[Redacted]' mask); 07-09 source-locality gate (no inviteCode literal in relay/src/) preserved by deviating from plan's locked path set — used 'code'/'*.code' wildcard in source while keeping host-side field name in test logger only

**Wave 4** *(blocked on Wave 3)*:
- [x] 07-05b Host-side cloud wiring + CloudHostTransport demultiplexer (SessionHostFactory.createCloud, session-register frame, per-joiner JWT issuance, envelope.target unicast) — commits 6ebafa0 RED + 1a982ed GREEN(1/3) + cf9283e GREEN(2/3) + 339e0ff GREEN(3/3); 31 new tests (12 cloudHostDemux + 11 hostCloudWiring + 8 hostRegister); 996 extension / 71 relay all green; SC-1 + SC-2 paper-verifiable; merge invariant — SessionHost has 0 matches for cloudMode/setCloudMode/handleCloudInboundFrame/VirtualConnection (intermediate dead-code state eliminated); T-07-09 + T-07-05 + T-07-spoof-member + T-07-02-exception + T-07-system-frame + T-07-collision + T-07-VC-leak all pinned to tests; 07-02 envelope byte-shape snapshot preserved byte-for-byte (target field optional, JSON.stringify omits undefined); 7 deviations all auto-fixed — _testClientTransport ctor seam (no real WSS in tests), _setDemuxLoggerForTest module seam (VS Code extension host console.log non-overridable from JS), test source-grep paths use process.cwd() not __dirname, _testHostJwt field for claims assertion, SessionHostFactory.ts stub during GREEN part 1 to unblock mocha loadFiles, comment paraphrasing for word-boundary gates, handleInbound parses PAYLOAD bytes not envelope; relay first-frame carve-out closes 4400 on member-role + session-register (T-07-09 ban on Phase 4.1 anti-pattern); envelope.target unicast preserves T-07-02 (zero .payload reads in router.ts)
- [x] 07-12 Deployment + docs (relay/Dockerfile, relay/fly.toml, relay/README.md, top-level README cloud section) — commits d7a42b2 (Task 1 feat: Dockerfile + fly.toml) + 1e7e006 (Task 2 docs: Deploy Your Relay quickstart) + 29d812f (Task 3 docs: top-level README cloud section); 274 doc/config lines added across 4 files; zero source code touched; 71/71 relay tests re-confirmed; extension count preserved at 996/0/66 via code-isolation; Dockerfile multi-stage on node:20-alpine with USER node non-root runtime (ASVS V14.1.1), EXPOSE 8080, CMD matches relay/package.json start verbatim; fly.toml: shared-cpu-1x 256mb in iad, force_https=true, auto_stop_machines="off" (sessions stateful — RESEARCH §Pitfall 1; T-07-21 mitigation), auto_start_machines=true (manual-stop/auto-wake pairing per CONTEXT D-03; planning_context wins over RESEARCH §A4 discrepancy via plan <read_first> line 153), min_machines_running=1, /healthz check every 30s, zero secret-shaped lines (T-07-22 mitigation, ASVS V14.3); relay/README.md 189 lines: 6-step Fly.io quickstart + 9-env-var reference table + auto_stop warning + cost transparency (2024-10-07 free-tier discontinuation + ~$2-5/month, avoids stale CONTEXT D-03 "free tier covers small teams" wording per RESEARCH §Pitfall 2; T-07-23 mitigation) + troubleshooting close-code map + Deploy-elsewhere (AWS Fargate/Hetzner CX11/DigitalOcean App Platform — same Docker image) + Future roadmap (managed versioncon.dev + deferred L3 + SSO/MFA/audit/refresh seams shipped in 07-02 + 07-03) + bidirectional cross-links; top-level README.md +22 surgical lines (Edit, not Write) — new ## VersionCon for Cloud Teams H2 between Git Users and Development sections, zero changes to 5 pre-existing sections (git diff -U0 confirms pure additive); zero deviations — plan executed exactly as written; no CI workflow added (CONTEXT D-02 self-host-only); 3 MANUAL UAT items deferred to verifier (docker build smoke — daemon was offline; 5-min end-to-end Fly deploy walkthrough; two-machine cloud session SC-1+SC-2+SC-3 live test); NET-06 feature-complete; threats T-07-19 (Fly edge trust accepted/documented) + T-07-20 (USER node mitigated) + T-07-21 + T-07-22 + T-07-23 + T-07-24 (CI accept/oos) + T-07-25 (build perf accept) all covered
